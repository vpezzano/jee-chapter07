...
public class Main {
	...
	public static void main(String[] args) throws NamingException, InterruptedException, ExecutionException {
		...
		try (EJBContainer ec = EJBContainer.createEJBContainer(properties)) {
			...
			BookEJBNoView bookEJBNoView = (BookEJBNoView) ctx.lookup("java:global/classes/BookEJBNoView!ejb.BookEJBNoView");
			...
			CacheLocal cacheLocal = (CacheLocal) ctx.lookup("java:global/classes/CacheEJB!model.CacheLocal");
			...
			ItemLocalXml itemLocalXml = (ItemLocalXml) ctx.lookup("java:global/classes/ItemEJBXml!model.ItemLocalXml");
			System.out.println("Numer of items: " + itemLocalXml.getNumberOfItems());

			ItemLocal itemLocal = (ItemLocal) ctx.lookup("java:global/classes/ItemEJB!model.ItemLocal");
			Item item1 = new Item("Item1", "Item1", 12.5f);
			Item item2 = new Item("Item2", "Item2", 22.5f);
			itemLocal.addItem(item1);
			itemLocal.addItem(item2);
			System.out.println("Total price: " + itemLocal.getTotal());

			ItemEJB itemEJB = (ItemEJB) ctx.lookup("java:global/classes/ItemEJB!ejb.ItemEJB");
			itemEJB.addItem(item1);
			itemEJB.addItem(item2);
	        System.out.println("Number of items: " + itemEJB.getNumberOfItems());
		}
